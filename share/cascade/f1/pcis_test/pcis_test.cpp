...
int main(void) {
    ...
    int dth_fd = open("/dev/xdma0_c2h_0", O_RDONLY);
    if (dth_fd == -1) {
        printf("Could not open XDMA C2H device\n");
        exit(EXIT_FAILURE);
    }
    int htd_fd = open("/dev/xdma0_h2c_0", O_WRONLY);
    if (htd_fd == -1) {
    ...
    // Zero device
    do_io(htd_fd, true, (8<<20), (uint64_t{64}<<30), 0);
    ...
    for (int i = 0; i < num_modes; ++i) {
        ...
        for (uint64_t xfer_size = 512; xfer_size <= (1<<22); xfer_size *= 2) {
            uint64_t total_xfer = std::min(xfer_size * (1<<15), (uint64_t{64}<<30));
            ...
            for (uint64_t thread = 0; thread < num_threads; ++thread) {
                ...
                int thread_fd = thread_writing ? htd_fd : dth_fd;
