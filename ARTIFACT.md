...
- `cd cascade-f1/`
<!-- - `git checkout 6cdba314aa76a3010a2948c98c0f157ebb727f16` -->
...
- You can optionally choose to install Cascade / Synergy. If you don't install it, the main executables can be found in the `cascade-f1` folder under `./build/tools/`
...
These `vivado_server` instances will run in the foreground until killed with `ctrl-c` or `fuser -k <port>/tcp`. It can take a minute for the port to be freed by the OS after killing `vivado_server`, so starting a new server with the same port may result in it exiting unexpectedly.

The Verilog and data files for our benchmarks can be found in `share/cascade/test/benchmark`. Each the 6 main benchmarks can be run as follows:

    cd /home/centos/src/project_data/cascade-f1
    cascade --march regression/f1_minimal -e share/cascade/test/benchmark/adpcm/adpcm.v --compiler_port 9901 --enable_info --profile 2
